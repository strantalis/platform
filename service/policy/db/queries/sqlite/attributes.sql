...
WHERE
    ...
    (sqlc.narg('namespace_id') IS NULL OR ad.namespace_id = sqlc.narg('namespace_id')) AND 
    (sqlc.narg('namespace_name') IS NULL OR n.name = sqlc.narg('namespace_name')) 
...
WITH target_definition AS (
    ...
    WHERE fqns.fqn IN (SELECT fqns_vals.value FROM json_each(sqlc.arg('fqns')) AS fqns_vals(value)) 
        AND ad.active = TRUE
...
"values" AS (
	...
	WHERE (av.active = TRUE OR sqlc.arg('include_inactive_values') = TRUE)
...
VALUES (gen_random_uuid(), sqlc.arg('namespace_id'), sqlc.arg('name'), sqlc.arg('rule'), sqlc.arg('metadata'), sqlc.arg('allow_traversal'), STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'now'), STRFTIME('%Y-%m-%dT%H:%M:%fZ', 'now')) 
RETURNING id;
