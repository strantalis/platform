...
WHERE (sqlc.narg('namespace_id') IS NULL OR rmg.namespace_id = sqlc.narg('namespace_id')) 
...
WHERE (sqlc.narg('group_id') IS NULL OR m.group_id = sqlc.narg('group_id'))
...
WITH groups_cte AS (
    ...
    WHERE ns.name = sqlc.arg('namespace_name') AND g.name = sqlc.arg('group_name') 
)
SELECT
    m.id,
    JSON_BUILD_OBJECT('id', av.id, 'value', av.value, 'fqn', fqns.fqn) as attribute_value,
