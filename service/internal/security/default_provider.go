...
func (d *Default) EncryptAsymmetric(_ context.Context, alg string, key []byte, hash crypto.Hash, data, ephemeralKey []byte) ([]byte, []byte, error) {
	if alg == policy.Algorithm_ALGORITHM_RSA_2048.String() || alg == policy.Algorithm_ALGORITHM_RSA_4096.String() {
		pub, err := parsePEMPublicKey(key)
		if err != nil {
			return nil, nil, err
		}
		...
		label := []byte("")
		cipherText, err := rsa.EncryptOAEP(hash.New(), rand.Reader, rsaPub, data, label)
		return cipherText, nil, err
	}

	if alg == policy.Algorithm_ALGORITHM_EC_P256.String() || alg == policy.Algorithm_ALGORITHM_EC_P384.String() || alg == policy.Algorithm_ALGORITHM_EC_P521.String() {
		// The KeyRef contains the recipient's public key
		pub, err := parsePEMPublicKey(ephemeralKey)
		if err != nil {
			return nil, nil, err
		}
		...
		cipherText, err := d.EncryptSymmetric(context.Background(), dk, data)
		if err != nil {
			return nil, nil, err
		}
...
func (d *Default) DecryptAsymmetric(_ context.Context, alg string, key, cipherText, ephemeralKey []byte) ([]byte, error) {
	if alg == policy.Algorithm_ALGORITHM_RSA_2048.String() || alg == policy.Algorithm_ALGORITHM_RSA_4096.String() {
		priv, err := parsePEMPrivateKey(key)
		if err != nil {
			return nil, err
		}
		...
		label := []byte("")
		return rsa.DecryptOAEP(sha1.New(), rand.Reader, rsaPriv, cipherText, label)
	}
	...
	if alg == policy.Algorithm_ALGORITHM_EC_P256.String() || alg == policy.Algorithm_ALGORITHM_EC_P384.String() || alg == policy.Algorithm_ALGORITHM_EC_P521.String() {
		...
		return d.DecryptSymmetric(context.Background(), dk, cipherText)
	}
...
func (d *Default) UnwrapKey(ctx context.Context, wrappedKey, kek []byte) ([]byte, error) {
	if len(wrappedKey) == 0 {
